    ...
    <header className="flex items-center justify-between px-6 py-4 border-b border-[#21344a] bg-[#101923] shrink-0">
      <div className="flex items-center gap-4">
        <div className="w-8 h-8 text-primary">
          {/* Splice Logo Mark */}
          <svg aria-hidden="true" className="fill-current w-full h-full" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
      ...
      <nav aria-label="Actions principales" className="flex items-center gap-3">
        ...
        {currentScreen === 'preview' && onBackToEditor && (
          <Button variant="outline" size="sm" onClick={onBackToEditor}>
            <ArrowLeft aria-hidden="true" className="w-4 h-4 mr-1.5" />
            Retour à l'éditeur
          </Button>
        )}
        ...
        {currentScreen === 'editor' && canPreview && onPreview && (
          <Button size="sm" variant="default" onClick={onPreview} disabled={isPreparingPreview}>
            {isPreparingPreview ? (
              <>
                <div aria-hidden="true" className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-1.5" />
                Préparation...
              </>
            ) : (
              <>
                <Play aria-hidden="true" className="w-4 h-4 mr-1.5" />
                Prévisualiser
              </>
            )}
          </Button>
